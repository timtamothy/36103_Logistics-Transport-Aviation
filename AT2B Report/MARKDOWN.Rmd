...
knitr::include_graphics(here::here("others", "q1.png"))
```
...
knitr::include_graphics(here::here("others", "mac.png"))
```
...
## How is American Airlines on-time performance compared to our two main competitors?
...
The main origins and destinations are states where these airlines have hubs, the top 16 states containing all the hubs from the 3 main airlines represent almost 89% of air traffic and therefore most of the delays occur within these busy airports (Radka, 2021). For that reason, we first decided to focus on the busiest airports and subset our model. Finally, we made the decision to subset our model exclusively to California, since it was the second busiest state, contains a higher average departure delay, and all three airlines have a primary hub located here. This model will then be used to generalise on-time performance across other hub states.
...
```{r add_picturepc, echo=FALSE}
knitr::include_graphics(here::here("others", "pc.png"))
```
...
```{r add_picturepcm, echo=FALSE}
knitr::include_graphics(here::here("others", "pcm.png"))
```
...
Chi-square test is a statistical hypothesis test which is non-parametric, meaning that it does not assume any underlying distribution, for example, linearity (McHugh, 2013).
...
```{r add_picturechi, echo=FALSE}
knitr::include_graphics(here::here("others", "chi.png"))
```
...
knitr::include_graphics(here::here("LM Plots", "normal_qq.png"))
knitr::include_graphics(here::here("LM Plots", "residual_vfitted.png"))
knitr::include_graphics(here::here("LM Plots", "scale_location.png"))
knitr::include_graphics(here::here("LM Plots", "cooks_distance.png"))
``` 
...
```{r add_binomail_plot, echo=FALSE, warning=F, message=FALSE}

library(knitr)
library(flextable)
confusion_aa <- read.table('confusion_aa.txt')
confusion_aa %>% regulartable() %>% autofit() %>% 
  width(j=~X0, width = 2) %>% width(j=~X1, width=2)
...
In conclusion, we have generated insights on how different variables influence departure delays . Our models provides a preliminary idea of what might have constituted departure delays in California. However, linear or logistics models conducted would not be advised to use as a prediction of departure delays in the future. As a result, other statistical modelling such as ARIMA should be supplemented in order to generate accurate and reliable insights and predictions for departure delays which can then be generalized as a prediction algorithm for other states.
...
```{r api, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(httr)
library(jsonlite)
library(dplyr)
icao <- GET(
...
```{r scape, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
# Explicit URL
...
```{r merge, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
# Load Files ----
...
```{r timedata, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r dot, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r mergedata, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r mergedata2, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r adding, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
maindf <- read_feather(here('clean_3airlines.feather'))
delta <- read_feather(here('DAL_fleet.feather'))
american <- read_feather(here('AA_fleet.feather'))
united <- read_feather(here('UAL_fleet.feather'))
...
```{r infoadd, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(feather)
library(here)
library(tidyverse)
library(future)
...
Adding Latitude and Longitude information by state into dataset
```{r organise, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(feather)
library(here)
library(tidyverse)
...
```{r employee, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
Plotting 
```{r plot, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
More EDA
```{r edaeda, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(tidyverse)
library(here)
...
```{r Correlation Test Code , message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
 library(tidyverse)
library(here)
library(readr)
library(future.apply)
...
Clean data
```{r clean, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r chisquare, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r Linear Model Code, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(tidyverse)
library(here)
library(readr)
library(future.apply)
...
````{r Logistic Model Code, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(tidyverse)
library(here)
library(readr)
library(future.apply)
...
```{r add_picturearrivaldelay, echo=FALSE}
knitr::include_graphics(here::here("EDA Plots", "arrival_delay_by_taxi_in.png"))
```
...
```{r add_pituretaxiin, echo=FALSE}
knitr::include_graphics(here::here("EDA Plots", "arrival_delay_by_taxi_out.png"))
```
...
```{r summary, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
library(funModeling)
library(Hmisc)
library(DataExplorer)
...
knitr::include_graphics(here::here("others", "summary.jpg"))
``` 
